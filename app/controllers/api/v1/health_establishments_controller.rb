  ...
  module V1
    class HealthEstablishmentsController < ApplicationController
      ...
      def filtered_establishments
        scope = HealthEstablishment.active.includes(:neighborhood, :establishment_services, :establishment_equipments)
        ...
        scope = scope.by_management(params[:management]) if params[:management].present?
        ...
        scope = scope.with_service(params[:service]) if params[:service].present?
        scope = scope.with_equipment(params[:equipment]) if params[:equipment].present?
